...
def home():
    ...
    for stock in unique_stock_list:
        stock_data = cursor.execute("select * from stock where stock_id=?", (stock, ))
        stock_single_info = stock_data.fetchall()
        stock_cost = 0 # 單一股票總花費
        shares = 0     # 單一股票股數
        for d in stock_single_info:
            shares += d[2]
            stock_cost += d[2] * d[3] + d[4] + d[5]
        # 取得目前股價
        url = "https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&stockNo=" + stock
        response = requests.get(url)
        stock_all_info = response.json()
        price_array = stock_all_info['data']
